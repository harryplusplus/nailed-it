...
| 개념           | 값                                                       |
| -------------- | -------------------------------------------------------- |
| 에이전트 ID    | `HINDSIGHT_AGENT_ID` 환경변수 (예: `coding`, `research`) |
...
const DEFAULT_AGENT_ID = 'coding' // HINDSIGHT_AGENT_ID 미설정 시 기본값
...
에이전트 ID를 환경변수 `HINDSIGHT_AGENT_ID`로 받아 bank ID를 구성:

```
HINDSIGHT_AGENT_ID=coding  →  bank: pi-coding
HINDSIGHT_AGENT_ID=research →  bank: pi-research
```
...
`AGENT_PROFILES` 상수에 에이전트별 mission/disposition 정의. `HINDSIGHT_AGENT_ID`가 프로필에 없으면 기본값으로 bank 생성.

```bash
# 코딩 에이전트로 Pi 실행
HINDSIGHT_AGENT_ID=coding pi

# 리서치 에이전트로 Pi 실행
HINDSIGHT_AGENT_ID=research pi
```
...
pi.on('session_start', async (event, ctx) => {
  const agentId = process.env.HINDSIGHT_AGENT_ID ?? DEFAULT_AGENT_ID
...
| 5   | ~~에이전트 프로필 자동 감지?~~                    | ✅ 해결: `HINDSIGHT_AGENT_ID` 환경변수로 명시적 선택                                                |
