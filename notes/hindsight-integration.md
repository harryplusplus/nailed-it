...
**사용하는 Hindsight 연산:** `retain` (기억 저장), `recall` (기억 검색)
**사용하지 않는 연산:** `reflect` — 질의응답 시나리오용 agentic 루프. 본 설계의 목적은 에이전트의 장기 기억으로 작업/행동 품질 향상이며, reflect의 disposition/mission은 reflect 호출 시에만 개입하므로 의미 없음.
...
const RUNTIME_PREFIX = 'pi' // document ID 접두사 (런타임별 분리: pi, oc, ...)

// ─── Bank Missions (retain/observations에만 영향, reflect 미사용) ───
...
| Mission | 대상 연산 | 현재 사용 여부 | 비고 |
|---------|----------|---------------|------|
| `retain_mission` | `retain` (팩트 추출) | ✅ 사용 | 팩트 추출 품질에 직접 영향. 가장 가치 있음. |
| `observations_mission` | observation consolidation (백그라운드) | ✅ 사용 | 패턴 합성 품질에 영향. 기억이 쌓이면 효과 체감. |
| `reflect_mission` | `reflect` (동기 agentic 루프) | ❌ 미사용 | reflect 호출 시에만 개입. |

**Disposition (skepticism/literalism/empathy):** reflect에만 영향. 현재 reflect 미사용으로 설정 불필요.
...
`AGENT_PROFILES` 상수에 에이전트별 `retain_mission`/`observations_mission` 정의. `NI_AGENT_ID`가 프로필에 없으면 기본값으로 bank 생성.
...
| -------------------- | -------------------------- | ------------------------- |
| `session_start`      | bank 생성/확인             | `createBank` + `updateBankConfig` |
...
pi.on('session_start', async (event, ctx) => {
  ...
  try {
    ...
    // retain_mission / observations_mission 설정
